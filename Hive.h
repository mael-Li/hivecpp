...
private:
    ...
    void dfsExplore(const HexCoord& current,std::unordered_set<HexCoord>& visited,const std::unordered_set<HexCoord>& occupied) const {
    ...
    //限制在4个回合之内必须下蜂后
        ...
        const std::unordered_map<PlayerID, HexCoord> & getqueenBeePositions()const{return queenBeePositions;}
private:
    // 检查是否需要强制放置蜂后
